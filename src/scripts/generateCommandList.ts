...
function subCommandItem(cmd: TCommand) {
  ...
  return `## ${topLevelName}${groupName ? `-> ${groupName}` : ""}${
    topLevelName ? " -> " : ""
  }${cmd.name}
...
**Options:** ${
    cmd.options.options
      ? cmd.options.options
          .map((v) => {
            const requiredText =
              "required" in v && v.required ? "Required" : "Optional";
...
function makeUsage(
  ...
  return `/${topLevelName ? `${topLevelName} ` : ""}${
    groupName ? `${groupName} ` : ""
  }${name} ...`;
}
