...
export const pagination = async (options: PaginationOptions) => {
  ...
  if (!interaction && !message)
    throw new Error(
      "Pagination requires either an interaction or a message object"
    );
  ...
  const resolveButtonName = (type: ButtonsTypes) => {
    return (Object.keys(ButtonTypes) as (keyof typeof ButtonTypes)[]).find(
      (key) => {
  ...
  const generateButtons = (state?: boolean) => {
    ...
    return names.reduce((accumulator: ButtonBuilder[], type: ButtonsTypes) => {
      let embed = new ButtonBuilder()
        ...
        .setStyle(
          getButtonData(type)?.style || defaultStyles[resolveButtonName(type)]
        );
      if (getButtonData(type)?.emoji !== null)
        embed.setEmoji(
          getButtonData(type)?.emoji || defaultEmojis[resolveButtonName(type)]
        );
  ...
  if (type === "interaction" && channel) {
    if (goBack) {
      ...
      await interaction!
        .deferReply({ ephemeral: ephemeralMessage })
        .catch(() => ({}));
    }
  ...
  const defaultFilter = (interaction: ButtonInteraction) => {
    return (
      interaction.user.id === author.id && parseInt(interaction.customId) <= 4
    );
  };
  ...
  if (pageTravel) {
    collectorModal = initialMessage.createMessageComponentCollector(
      collectorOptions(
        (_i: ModalSubmitInteraction) =>
          _i.user.id === author.id && parseInt(_i.customId) === 5
      )
    );
          ...
          if (err.code === 50027) {
            console.log(
              `Webhook token has expired : ${
                client
                  ? "trying to delete embed from it's id..."
                  : "no client found in option, cannot delete embed"
              }`
            );
            if (client) {
              client.channels
                ...
                .then(() => {
                  console.log(
                    `Message with ${initialMessage.id} id has been deleted successfully.`
                  );
                })
                .catch((err) =>
                  console.log(
                    `Channel does not exist or other error has been raised :\n${err}`
                  )
                );
            }
          }
